...
def add_text(
    ...
    subtitle_text = f"{date} - {text_name}" # Subtext below title of figures
    fig.text(
        0.5, subtitle_position, subtitle_text, fontsize=18, ha="center", va="bottom"
    )

    tag_text = f"Generated by the Glider Guidance System 3 (GGS3)"
    fig.text(
        0.5,
        tag_position,
        tag_text,
...
def create_glider_path(ax: object, path: list, waypoints: list, color="k") -> tuple:
    ...
    wp_plot = ax.scatter(  # waypoint toggles
        wp_lons,
        wp_lats,
        color="white",  #changing waypoints to white: increased visibility // intermediate waypoints
        edgecolors="black",
        marker="o",
        transform=ccrs.PlateCarree(),
        # s=30, might change the size of the marker, would probably lose some detail in the plotline, color might be enough for vis
        zorder=51,
    )
...
def populate_map(
    ...
    **kwargs,
    ...
    # Create contours.
    # Adjusted figure captions for magnitutde plotting
    if contour_type == "magnitude":
        ...
        ## integration
    elif contour_type=='DRIFT_SUMMARY':
        plot_title='Drift summary(Cross-track current along optimal path)'
        label=r'cross-track current $u_\perp$ ($\mathregular{m\,s^{-1}}$)'
        if not hasattr(model, 'drift_records') or not model.drift_records:
            print('[DRIFT] No drift_records on model; skipping drift_summary plot.')
        else:
            recs=model.drift_records
            lon_mid=np.array([r['lon_mid']for r in recs])
            lat_mid=np.array([r['lat_mid'] for r in recs])

            vals = np.array([r["u_perp_mps"] for r in recs])

        # Symmetric color limits so sign is obvious
        ...
        sc = ax.scatter(
            ...
            c=vals,
            s=15,
            cmap=cmo.thermal,  # or "coolwarm" if you prefer
            vmin=-vmax,
            vmax=vmax,
            transform=ccrs.PlateCarree(),
            edgecolor="none",
        ...
        cax = create_cbar(fig, ax, sc, label)
    ...
    elif contour_type == "mean_magnitude": ## color bar changing 
    ...
    # Debug start
    #print("\n---DEBUGGING: Inspecting 'data' object ---")
    #print(data)
    #print ("\nAvailable coordinates in 'data.coords':")
    #print(data.coords)
    #print("---Debugging: End---\n")
    # Debugging end

    #date = data.time.dt.strftime("%Y-%m-%d %H:%M").values
   # date = "2025-1st-November " # adjustable date that appears on figure
...
def create_map(
    ...
    **kwargs,
    ...
    if save:
        ...
        #ddate = "2025-11-01 (plot date)"       #data.time.dt.strftime("%Y_%m_%d").values // folder name
        ddate = dt.datetime.now(dt.timezone.utc).strftime("%Y-%m-%d (plot date)")
        #fdate = "2025-11-01"                    # file name 
        fdate = dt.datetime.now(dt.timezone.utc).strftime("%Y-%m-%d")
